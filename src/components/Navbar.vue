  ...
  >
    <nav class="flex:none lg:hidden">
      <label @click="$emit('toggle-drawer')" class="btn btn-square btn-ghost">
        <Icon
          icon="line-md:close-to-menu-alt-trasition"
    ...
    <section class="flex-1">
      <div class="form-control">
        <input
          type="text"
          placeholder="search"
          class="input input-bordered w-48 l-2 md:w-auto dark:bg-white/5 dark:backdrop-blur-md dark:[--webkit-backdrop-filter:blur(10px)] dark:border-white/10"
        />
      </div>
    </section>
